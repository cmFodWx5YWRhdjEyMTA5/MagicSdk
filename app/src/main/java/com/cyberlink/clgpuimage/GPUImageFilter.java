...
//            "     gl_FragColor = vec4(1.0,0.0,0.0,1.0);\n" +
    ...
    public GPUImageFilter(final String vertexShader, final String fragmentShader) {
        this(vertexShader, fragmentShader, DEFAULT_VERTEXS, DEFAULT_FRAGS);
    }

    public GPUImageFilter(final String vertexShader,
                          final String fragmentShader,
                          final float[] vertexBuffer,
                          final float[] fragBuffer) {
        mRunOnDraw = new LinkedList<Runnable>();
        mVertexShader = vertexShader;
        mFragmentShader = fragmentShader;
        mVertexBuffer = ByteBuffer.allocateDirect(vertexBuffer.length * 4).order(ByteOrder.nativeOrder()).asFloatBuffer();
        mVertexBuffer.put(vertexBuffer);
        mVertexBuffer.position(0);
        mFragVertexBuffer = ByteBuffer.allocateDirect(fragBuffer.length * 4).order(ByteOrder.nativeOrder()).asFloatBuffer();
        mFragVertexBuffer.put(fragBuffer);
        mFragVertexBuffer.position(0);
    }
    ...
    public void onDraw(final int textureId) {
        onDraw(textureId, mVertexBuffer, mFragVertexBuffer);
    }
